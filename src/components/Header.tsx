    ...
    <>
      <header className={`${isDark ? 'bg-zinc-900/95 border-yellow-400/10' : 'bg-white/95 border-slate-200'} backdrop-blur-xl shadow-sm sticky top-0 z-50 border-b transition-colors duration-300`}>
        <div className={`${isBoxed ? 'max-w-7xl' : 'max-w-[1920px]'} mx-auto px-3 sm:px-6 py-2`}>
          ...
          {/* ── Mobile search mode ─────────────────────────────────────── */}
          {mobileSearchOpen && (
            <div className="flex items-center gap-2 md:hidden py-0.5">
              <button
                onClick={() => { setMobileSearchOpen(false); setSearchQuery(''); }}
                className={`p-2 rounded-lg transition-colors shrink-0 ${isDark ? 'text-slate-400 hover:text-white hover:bg-white/5' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'}`}
              >
          ...
          <div className={`items-center justify-between gap-3 md:gap-6 ${mobileSearchOpen ? 'hidden md:flex' : 'flex'}`}>
            ...
            <div className="flex items-center gap-2 sm:gap-3 lg:gap-4 text-sm font-bold shrink-0">
              ...
              {/* Mobile search icon */}
              <button
                className={`md:hidden p-2 rounded-lg transition-all ${isDark ? 'text-slate-400 hover:bg-white/5 hover:text-white' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'}`}
                  ...
                  >
                    ...
                    <div className="hidden sm:flex flex-col text-left">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className={`text-xs font-bold leading-tight max-w-[80px] truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>
                          {user?.name}
                        </span>
                      </div>
                    </div>
              ...
              <Link to="/cart" className="relative flex items-center gap-2 cursor-pointer group">
                <div className="relative">
                  ...
                  {totalItems > 0 && (
                    <span className={`absolute -top-1.5 -right-1.5 ${isDark ? 'bg-white text-black' : 'bg-slate-900 text-white'} text-[10px] font-bold px-1.5 py-0.5 rounded-full ring-2 ${isDark ? 'ring-zinc-900' : 'ring-white'}`}>
